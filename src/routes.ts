...
async function handleAuthLogin(req: Request): Promise<Response> {
  try {
    ...
    const users = await db`SELECT id, email, password_hash, full_name FROM public.hermes_users WHERE email = ${email}`.execute();
    ...
    const roles = await db`SELECT role FROM public.hermes_user_roles WHERE user_id = ${user.id}`.execute();
...
async function handleAdminListUsers(req: Request): Promise<Response> {
  try {
    const users = await db`SELECT u.id, u.email, COALESCE(u.full_name, '') as full_name, COALESCE(ur.role, 'user') as role FROM public.hermes_users u LEFT JOIN public.hermes_user_roles ur ON u.id = ur.user_id WHERE u.disabled = false ORDER BY u.created_at DESC LIMIT 50`.execute();
...
async function handleAdminSetRole(req: Request): Promise<Response> {
  try {
    ...
    await db`INSERT INTO public.hermes_user_roles (user_id, role, created_at, updated_at) VALUES (${user_id}::uuid, ${role}::cafe.app_role, NOW(), NOW()) ON CONFLICT (user_id) DO UPDATE SET role = ${role}::cafe.app_role, updated_at = NOW()`.execute();
