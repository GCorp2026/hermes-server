...
export async function handleEmailHistory(req: Request) {
  ...
  return withAuthContext(userId, async (sql) => {
    ...
    const rows = await sql`SELECT * FROM email_history WHERE work_email_id = ${workEmailId} ORDER BY created_at DESC`.execute();
    return json(rows);
  });
}
...
export async function handleLabelAssignments(req: Request) {
  ...
  return withAuthContext(userId, async (sql) => {
    const rows = await sql`SELECT ela.* FROM email_label_assignments ela JOIN work_emails we ON ela.work_email_id = we.id WHERE we.user_id = ${userId}`.execute();
    return json(rows);
  });
}
...
export async function deleteLabelAssignment(req: Request) {
  ...
  return withAuthContext(userId, async (sql) => {
    ...
    await sql`DELETE FROM email_label_assignments WHERE email_history_id = ${emailHistoryId} AND label_id = ${labelId}`.execute();
    return json({ success: true });
  });
}
...
export async function handleSendEmail(req: Request) {
  ...
  return withAuthContext(auth.userId, async (sql) => {
    ...
    // Immediate send via Resend API directly
...
// GET /api/templates
export async function listTemplates(req: Request, userId: string) {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders() });
  if (!userId) return error("Unauthorized", 401);
  return withAuthContext(userId, async (sql) => {
    const rows = await sql`SELECT id, user_id, name, subject, body, created_at, updated_at FROM email_templates WHERE user_id = ${userId} ORDER BY created_at DESC`.execute();
    return json(rows);
  });
}

// POST /api/templates
export async function createTemplate(req: Request, userId: string) {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders() });
  if (!userId) return error("Unauthorized", 401);
  const { name, subject, body } = await req.json();
  if (!name) return error("name is required");
  return withAuthContext(userId, async (sql) => {
    const [row] = await sql`INSERT INTO email_templates (user_id, name, subject, body) VALUES (${userId}, ${name || ""}, ${subject || ""}, ${body || ""}) RETURNING *`.execute();
    return json(row, 201);
  });
}

// PUT /api/templates/:id
export async function updateTemplate(req: Request, templateId: string, userId: string) {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders() });
  if (!userId) return error("Unauthorized", 401);
  if (!templateId) return error("template id is required");
  const { name, subject, body } = await req.json();
  return withAuthContext(userId, async (sql) => {
    const [row] = await sql`UPDATE email_templates SET name = COALESCE(${name}, name), subject = COALESCE(${subject}, subject), body = COALESCE(${body}, body), updated_at = NOW() WHERE id = ${templateId} AND user_id = ${userId} RETURNING *`.execute();
    return json(row || null);
  });
}

// DELETE /api/templates/:id
export async function deleteTemplate(req: Request, templateId: string, userId: string) {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders() });
  if (!userId) return error("Unauthorized", 401);
  if (!templateId) return error("template id is required");
  return withAuthContext(userId, async (sql) => {
    await sql`DELETE FROM email_templates WHERE id = ${templateId} AND user_id = ${userId}`.execute();
    return json({ success: true });
  });
}
